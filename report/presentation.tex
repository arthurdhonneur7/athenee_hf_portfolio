
\documentclass[11pt]{article}
\usepackage[utf8]{inputenc}
\usepackage[left=0.2in, right=0.2in, top=0.4in, bottom=0.8in]{geometry}
\usepackage{graphicx}
\usepackage{booktabs}
\usepackage{xcolor}
\usepackage{colortbl}
\usepackage{tabularx}
\usepackage{tcolorbox}
\usepackage{titlesec}
\usepackage{microtype}
\usepackage{helvet}
\usepackage{hyperref}

\renewcommand{\familydefault}{\sfdefault}

\definecolor{primary}{RGB}{15, 23, 42}
\definecolor{accent}{RGB}{37, 99, 235}
\definecolor{cardbg}{RGB}{248, 250, 252}
\definecolor{cardborder}{RGB}{226, 232, 240}
\definecolor{posgreen}{RGB}{34, 197, 94}
\definecolor{negred}{RGB}{239, 68, 68}

\titleformat{\section}
  {\normalfont\Large\bfseries\color{primary}}
  {}{0em}
  {\color{accent}\vrule width 4pt\hspace{6pt}\color{primary}}

\title{\vspace{-2cm}\color{primary}\textbf{Fund of Hedge Funds Strategy Overview}}
\author{}
\date{\vspace{-1.5cm}}

\begin{document}
\maketitle

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% PAGE 1: 6-Month Strategy Deep Dive
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section*{Athenee Lazarus Alpha AMC}

\begin{tcolorbox}[boxrule=0pt, leftrule=4pt, colframe=primary, colback=cardbg, sharp corners]
\textbf{\large Executive Summary} \\[4pt]
This product is a fund of hedge funds. The core idea is to pick strong performers that are uncorrelated, ensuring that returns keep up while overall volatility and max drawdown are significantly reduced. By blending distinct, non-overlapping strategies, the portfolio aims for robust risk-adjusted returns across market cycles.
\end{tcolorbox}

\subsection*{Monthly Returns Breakdown}
\begin{table}[h]
\centering
\renewcommand{\arraystretch}{1.3}
\begin{tabularx}{\textwidth}{l *{13}{>{\centering\arraybackslash}X}}
\toprule
\textbf{Year} & \textbf{Jan} & \textbf{Feb} & \textbf{Mar} & \textbf{Apr} & \textbf{May} & \textbf{Jun} & \textbf{Jul} & \textbf{Aug} & \textbf{Sep} & \textbf{Oct} & \textbf{Nov} & \textbf{Dec} & \textbf{YTD} \\
\midrule
2022 & \cellcolor{posgreen!2!white}0.3\% & \cellcolor{posgreen!25!white}2.6\% & \cellcolor{posgreen!35!white}3.6\% & \cellcolor{posgreen!21!white}2.2\% & \cellcolor{posgreen!36!white}3.6\% & \cellcolor{posgreen!28!white}2.8\% & \cellcolor{posgreen!35!white}3.6\% & \cellcolor{posgreen!33!white}3.3\% & \cellcolor{posgreen!20!white}2.0\% & \cellcolor{posgreen!24!white}2.5\% & \cellcolor{posgreen!21!white}2.1\% & \cellcolor{posgreen!33!white}3.4\% & \cellcolor{posgreen!100!white}37.1\% \\
2023 & \cellcolor{posgreen!20!white}2.1\% & \cellcolor{posgreen!6!white}0.6\% & \cellcolor{posgreen!21!white}2.1\% & \cellcolor{posgreen!20!white}2.0\% & \cellcolor{posgreen!40!white}4.1\% & \cellcolor{posgreen!28!white}2.9\% & \cellcolor{posgreen!29!white}2.9\% & \cellcolor{posgreen!11!white}1.2\% & \cellcolor{posgreen!13!white}1.3\% & \cellcolor{posgreen!9!white}1.0\% & \cellcolor{posgreen!37!white}3.8\% & \cellcolor{posgreen!22!white}2.2\% & \cellcolor{posgreen!100!white}29.5\% \\
2024 & \cellcolor{posgreen!24!white}2.4\% & \cellcolor{posgreen!22!white}2.2\% & \cellcolor{posgreen!36!white}3.6\% & \cellcolor{posgreen!11!white}1.1\% & \cellcolor{posgreen!14!white}1.5\% & \cellcolor{posgreen!18!white}1.9\% & \cellcolor{posgreen!27!white}2.8\% & \cellcolor{posgreen!17!white}1.8\% & \cellcolor{posgreen!15!white}1.6\% & \cellcolor{posgreen!18!white}1.8\% & \cellcolor{posgreen!33!white}3.4\% & \cellcolor{posgreen!8!white}0.9\% & \cellcolor{posgreen!100!white}28.0\% \\
2025 & \cellcolor{posgreen!16!white}1.6\% & \cellcolor{posgreen!23!white}2.3\% & \cellcolor{posgreen!19!white}1.9\% & \cellcolor{posgreen!19!white}2.0\% & \cellcolor{posgreen!1!white}0.2\% & \cellcolor{posgreen!10!white}1.1\% & \cellcolor{posgreen!30!white}3.0\% & \cellcolor{posgreen!24!white}2.5\% & \cellcolor{posgreen!31!white}3.2\% & \cellcolor{posgreen!38!white}3.9\% & \cellcolor{posgreen!28!white}2.8\% & \cellcolor{posgreen!14!white}1.5\% & \cellcolor{posgreen!100!white}29.2\% \\
2026 & \cellcolor{posgreen!15!white}1.6\% & \cellcolor{posgreen!16!white}1.6\% & \cellcolor{posgreen!14!white}1.4\% & \cellcolor{posgreen!7!white}0.8\% & - & - & - & - & - & - & - & - & \cellcolor{posgreen!55!white}5.6\% \\
\bottomrule
\end{tabularx}
\end{table}

\subsection*{Strategy Statistics}
\begin{table}[h]
\centering
\renewcommand{\arraystretch}{1.3}
\scriptsize
\begin{tabularx}{\textwidth}{l *{12}{>{\centering\arraybackslash}X}}
\toprule
 & \textbf{CAGR} & \textbf{Std} & \textbf{Proxy Sharpe} & \textbf{MDD} & \textbf{Calmar} & \textbf{Pos Months} & \textbf{Up Capture} & \textbf{Down Capture} & \textbf{Alpha (t-stat)} & \textbf{Beta} & \textbf{Corr SP500} & \textbf{N months}\\
\midrule
\textbf{Lazarus Alpha} & 29.82\% & 3.38\% & 7.83 & 0.00\% & - & 100.0\% & 0.62 & -0.45 & 26.00\% (15.72) & 0.04 & 0.17 & 52 \\
\bottomrule
\end{tabularx}
\end{table}

\begin{figure}[h]
    \centering
    \includegraphics[width=0.95\textwidth, height=0.25\textheight, keepaspectratio]{growth_6_month_reb_initial_alloc_vs_sp500.pdf}
\end{figure}

\begin{figure}[h]
    \centering
    \includegraphics[width=0.95\textwidth, height=0.25\textheight, keepaspectratio]{scatter_6_month_reb_initial_alloc_vs_sp500.pdf}
\end{figure}



\clearpage
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% PAGE 2: Fund Allocation & Strategy Descriptions
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section*{Fund Allocation \& Strategy Descriptions}
The table below highlights the initial capital allocation weights alongside a brief description of each underlying fund's distinct edge and methodology.


\begin{tcolorbox}[colback=cardbg, colframe=cardborder, boxrule=0.5pt, arc=4pt, left=6pt, right=6pt, top=6pt, bottom=6pt]
    \textbf{\textcolor{primary}{Quanstream}} \hfill \textbf{\textcolor{accent}{100.0\%}} \\[4pt]
    \textcolor{primary}{Volatility seller across more than 150 commodities underlying with over 20 years of experience in these strategies.}
\end{tcolorbox}

\begin{tcolorbox}[colback=cardbg, colframe=cardborder, boxrule=0.5pt, arc=4pt, left=6pt, right=6pt, top=6pt, bottom=6pt]
    \textbf{\textcolor{primary}{M1}} \hfill \textbf{\textcolor{accent}{100.0\%}} \\[4pt]
    \textcolor{primary}{Crypto carry trade market neutral fund.}
\end{tcolorbox}

\begin{tcolorbox}[colback=cardbg, colframe=cardborder, boxrule=0.5pt, arc=4pt, left=6pt, right=6pt, top=6pt, bottom=6pt]
    \textbf{\textcolor{primary}{Vadantia}} \hfill \textbf{\textcolor{accent}{100.0\%}} \\[4pt]
    \textcolor{primary}{Trading-oriented fund on FX and precious metals with more than 4 trades per day.}
\end{tcolorbox}

\begin{tcolorbox}[colback=cardbg, colframe=cardborder, boxrule=0.5pt, arc=4pt, left=6pt, right=6pt, top=6pt, bottom=6pt]
    \textbf{\textcolor{primary}{Wizard Quant}} \hfill \textbf{\textcolor{accent}{100.0\%}} \\[4pt]
    \textcolor{primary}{Chinese equity market neutral fund that is strictly quant-driven.}
\end{tcolorbox}

\begin{tcolorbox}[colback=cardbg, colframe=cardborder, boxrule=0.5pt, arc=4pt, left=6pt, right=6pt, top=6pt, bottom=6pt]
    \textbf{\textcolor{primary}{ARR}} \hfill \textbf{\textcolor{accent}{100.0\%}} \\[4pt]
    \textcolor{primary}{Trading fund on equities and macro, utilizing a quantamental approach to identify equities with high upside, backed by a strict risk management policy.}
\end{tcolorbox}

\begin{tcolorbox}[colback=cardbg, colframe=cardborder, boxrule=0.5pt, arc=4pt, left=6pt, right=6pt, top=6pt, bottom=6pt]
    \textbf{\textcolor{primary}{R Squared}} \hfill \textbf{\textcolor{accent}{100.0\%}} \\[4pt]
    \textcolor{primary}{Equities futures arbitrage and dividend futures arbitrage relative value style fund.}
\end{tcolorbox}

\begin{tcolorbox}[colback=cardbg, colframe=cardborder, boxrule=0.5pt, arc=4pt, left=6pt, right=6pt, top=6pt, bottom=6pt]
    \textbf{\textcolor{primary}{Shiprock}} \hfill \textbf{\textcolor{accent}{100.0\%}} \\[4pt]
    \textcolor{primary}{Fund investing in highly distressed debt and bond contracts.}
\end{tcolorbox}

\begin{tcolorbox}[colback=cardbg, colframe=cardborder, boxrule=0.5pt, arc=4pt, left=6pt, right=6pt, top=6pt, bottom=6pt]
    \textbf{\textcolor{primary}{Smithson}} \hfill \textbf{\textcolor{accent}{100.0\%}} \\[4pt]
    \textcolor{primary}{Smithson fund description.}
\end{tcolorbox}

\begin{tcolorbox}[colback=cardbg, colframe=cardborder, boxrule=0.5pt, arc=4pt, left=6pt, right=6pt, top=6pt, bottom=6pt]
    \textbf{\textcolor{primary}{26 Miles}} \hfill \textbf{\textcolor{accent}{100.0\%}} \\[4pt]
    \textcolor{primary}{Indian-based fund investing in medium-frequency strategies across a lot of asset classes (intra-day trading with holdings of 2-3 hours). Founded by a team with strong prop trading desk experience who spun out to create this fund.}
\end{tcolorbox}

\begin{tcolorbox}[colback=cardbg, colframe=cardborder, boxrule=0.5pt, arc=4pt, left=6pt, right=6pt, top=6pt, bottom=6pt]
    \textbf{\textcolor{primary}{Adar1}} \hfill \textbf{\textcolor{accent}{100.0\%}} \\[4pt]
    \textcolor{primary}{Adar1 fund description.}
\end{tcolorbox}

\begin{tcolorbox}[colback=cardbg, colframe=cardborder, boxrule=0.5pt, arc=4pt, left=6pt, right=6pt, top=6pt, bottom=6pt]
    \textbf{\textcolor{primary}{Sam Capital}} \hfill \textbf{\textcolor{accent}{100.0\%}} \\[4pt]
    \textcolor{primary}{Sam capital fund description.}
\end{tcolorbox}


\section*{Detailed Fund Statistics}
\begin{table}[h]
\centering
\renewcommand{\arraystretch}{1.3}
\scriptsize
\begin{tabularx}{\textwidth}{l *{12}{>{\centering\arraybackslash}X}}
\toprule
 & \textbf{CAGR} & \textbf{Std} & \textbf{Proxy Sharpe} & \textbf{MDD} & \textbf{Calmar} & \textbf{Pos Months} & \textbf{Up Capture} & \textbf{Down Capture} & \textbf{Alpha (t-stat)} & \textbf{Beta} & \textbf{Corr SP500} & \textbf{N months}\\
\midrule
\textbf{Quanstream} & 21.50\% & 8.46\% & 2.36 & -9.18\% & 2.34 & 78.6\% & 0.51 & -0.36 & 18.82\% (9.26) & 0.09 & 0.17 & 215 \\
\textbf{M1} & 18.37\% & 5.43\% & 3.16 & -0.77\% & 23.91 & 92.5\% & 0.43 & -0.41 & 15.81\% (4.79) & 0.06 & 0.15 & 40 \\
\textbf{Vadantia} & 34.62\% & 18.63\% & 1.70 & -26.03\% & 1.33 & 69.6\% & 0.68 & -0.75 & 34.33\% (5.44) & -0.17 & -0.14 & 112 \\
\textbf{Wizard Quant} & 20.55\% & 13.63\% & 1.45 & -10.84\% & 1.90 & 65.2\% & 0.39 & -0.60 & 20.53\% (4.86) & -0.06 & -0.06 & 135 \\
\textbf{Arr} & 19.54\% & 16.97\% & 1.14 & -15.68\% & 1.25 & 59.6\% & 0.55 & -0.16 & 18.44\% (2.19) & 0.07 & 0.07 & 52 \\
\textbf{R Squared} & 18.18\% & 8.86\% & 1.94 & -4.78\% & 3.80 & 71.2\% & 0.48 & -0.16 & 15.35\% (3.80) & 0.13 & 0.24 & 59 \\
\textbf{Shiprock} & 25.85\% & 11.81\% & 2.02 & -8.74\% & 2.96 & 78.1\% & 0.68 & -0.03 & 20.12\% (4.08) & 0.19 & 0.26 & 73 \\
\textbf{Smithson} & 47.17\% & 6.69\% & 5.90 & -0.63\% & 74.87 & 96.9\% & 0.80 & -1.00 & 40.02\% (13.31) & -0.04 & -0.08 & 65 \\
\textbf{26 Miles} & 24.90\% & 7.36\% & 3.09 & -2.20\% & 11.33 & 84.4\% & 0.44 & -0.62 & 24.14\% (7.39) & -0.10 & -0.20 & 64 \\
\textbf{Adar1} & 34.92\% & 15.60\% & 2.02 & -10.63\% & 3.28 & 74.4\% & 0.88 & -0.18 & 26.38\% (4.60) & 0.31 & 0.34 & 86 \\
\textbf{Sam Capital} & 47.02\% & 14.44\% & 2.78 & -15.19\% & 3.10 & 86.8\% & 1.16 & -0.57 & 36.15\% (10.11) & 0.28 & 0.28 & 197 \\
\bottomrule
\end{tabularx}
\end{table}

\section*{Performance \& Correlation}
The following visuals illustrate the fund-level statistics and the diversification benefits.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.95\textwidth, height=0.25\textheight, keepaspectratio]{fund_stats.pdf}
\end{figure}

\begin{figure}[h]
    \centering
    \includegraphics[width=0.95\textwidth, height=0.25\textheight, keepaspectratio]{correlation.pdf}
\end{figure}

\begin{figure}[h]
    \centering
    \includegraphics[width=0.95\textwidth, height=0.25\textheight, keepaspectratio]{worst_sp500_months.pdf}
\end{figure}

\clearpage
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% PAGE 3: Portfolio Backtest Methodologies
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section*{Portfolio Growth Analysis}
Here we compare four portfolio weighting methodologies.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.95\textwidth, height=0.25\textheight, keepaspectratio]{4_strategies_growth.pdf}
\end{figure}



\end{document}
